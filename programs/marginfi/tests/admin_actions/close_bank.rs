...
async fn close_marginfi_bank() -> anyhow::Result<()> {
    ...
    let marginfi_group_f = &test_f.marginfi_group;
    ...
    // Attempt to close usdc_bank_f
    let res = marginfi_group_f
        .try_close_bank(usdc_bank_f)
        .await;
    ...
    // Attempt to close sol_bank_f
    let res = marginfi_group_f
        .try_close_bank(sol_bank_f)
        .await;

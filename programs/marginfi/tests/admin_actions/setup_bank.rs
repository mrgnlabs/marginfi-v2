...
async fn configure_bank_success(bank_mint: BankMint) -> anyhow::Result<()> {
    ...
    let _ = {
        ...
        let exp_config = interest_rate_config.clone().unwrap();
        let actual_config = bank.config.interest_rate_config;
        assert_eq!(exp_config.insurance_fee_fixed_apr, actual_config.insurance_fixed_fee.into());
        assert_eq!(exp_config.insurance_ir_fee, actual_config.insurance_rate_fee.into());
        assert_eq!(exp_config.group_fixed_fee_apr, actual_config.group_fixed_fee.into());
        assert_eq!(exp_config.group_ir_fee, actual_config.group_rate_fee.into());

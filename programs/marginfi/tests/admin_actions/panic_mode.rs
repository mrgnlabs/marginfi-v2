...
async fn test_panic_pause_and_unpause_instructions() -> anyhow::Result<()> {
    ...
    assert!(!fee_state.panic_state.is_paused());
...
async fn test_panic_state_cache_with_instructions() -> anyhow::Result<()> {
    ...
    let marginfi_group_state: MarginfiGroup = test_f.load_and_deserialize(&marginfi_group.key).await;
    ...
    assert!(!fee_state.panic_state.is_expired(current_time + PanicState::PAUSE_DURATION_SECONDS - 1));
    assert!(fee_state.panic_state.is_expired(current_time + PanicState::PAUSE_DURATION_SECONDS));

...
impl TestFixture {
    pub async fn new(test_settings: Option<TestSettings>) -> TestFixture {
        ...
        program.add_account(
            PYTH_USDC_FEED,
            craft_pyth_price_account(usdc_keypair.pubkey(), 1, USDC_MINT_DECIMALS.into(), None),
        );
        program.add_account(
            PYTH_SOL_FEED,
            craft_pyth_price_account(sol_keypair.pubkey(), 10, SOL_MINT_DECIMALS.into(), None),
        );
        program.add_account(
        ...
        program.add_account(
            PYTH_MNDE_FEED,
            craft_pyth_price_account(mnde_keypair.pubkey(), 10, MNDE_MINT_DECIMALS.into(), None),
        );

...
describe("Pyth pull oracles in localnet", () => {
    ...
    for (let i = 1; i < banks.length; i += 1) {
      ...
      tx.add(
        ...
        await borrowIx(user.mrgnBankrunProgram, {
          ...
          bank: banks[i],
          tokenAccount: user.lstAlphaAccount,
          remaining: remainingAccounts.flat(),
          amount: borrowAmount,
        })
      );
      tx.recentBlockhash = await getBankrunBlockhash(bankrunContext);
  ...
  //   const liquidateeAcc = await bankrunProgram.account.marginfiAccount.fetch(liquidateeAccount);
  //   dumpAccBalances(liquidateeAcc);
  //   const liquidatorAcc = await bankrunProgram.account.marginfiAccount.fetch(liquidatorAccount);

...
describe("k15: Kamino - Marginfi Deposits & Withdrawals", () => {
    ...
    before(async () => {
        ...
        usdcBankObligation = kaminoAccounts.get(`${usdcBank.toString()}_OBLIGATION`)!;
        tokenABankObligation = kaminoAccounts.get(`${tokenABank.toString()}_OBLIGATION`)!;
    });

    describe("1. Instant Deposit/Withdraw Tests", () => {
        describe("1.1 Large Amount Tests ($1M)", () => {
            it("User A - Deposit and withdraw 1M USDC", async () => {
                const DEPOSIT_AMOUNT = new BN(1_000_000).mul(new BN(10 ** ecosystem.usdcDecimals));
                ...
                await processBankrunTransaction(bankrunContext, fundTx, [globalProgramAdmin.wallet]);
                ...
                const marginfiAccountBeforeDeposit = await userA.mrgnBankrunProgram.account.marginfiAccount.fetch(
                    userA.accounts.get(USER_ACCOUNT_K)!
                );

                const balanceBeforeDeposit = marginfiAccountBeforeDeposit.lendingAccount.balances.find(
                ...
                await processBankrunTransaction(bankrunContext, refreshTx, [globalProgramAdmin.wallet]);
                ...
                await processBankrunTransaction(bankrunContext, depositTx, [userA.wallet]);

                // Verify deposit
                const userBalanceAfterDeposit = await getTokenBalance(bankRunProvider, userA.usdcAccount);
                const marginfiAccountAfterDeposit = await userA.mrgnBankrunProgram.account.marginfiAccount.fetch(
                    userA.accounts.get(USER_ACCOUNT_K)!
                );

                const balanceAfterDeposit = marginfiAccountAfterDeposit.lendingAccount.balances.find(
                    b => b.active === 1 && b.bankPk.equals(usdcBank)
                )!;

                // Get collateral amount from marginfi balance difference (since user had existing deposits)
                const collateralSharesAfterDeposit = wrappedI80F48toBigNumber(balanceAfterDeposit.assetShares);
                const collateralDifference = collateralSharesAfterDeposit.minus(collateralSharesBefore);
                const collateralAmount = new BN(collateralDifference.toString());

                // Verify deposit
                assert.ok(userBalanceAfterDeposit === userBalanceBefore - DEPOSIT_AMOUNT.toNumber());
                ...
                console.log("DEBUG: Actual bank in account:", balanceAfterDeposit.bankPk.toString());
                console.log("DEBUG: Banks match:", usdcBank.equals(balanceAfterDeposit.bankPk));

                // Debug: Check ALL active balances on the account
                console.log("DEBUG: All active balances on account:");
                for (const [index, balance] of marginfiAccountAfterDeposit.lendingAccount.balances.entries()) {
                    if (balance.active === 1) {
                        ...
                        console.log(`  Balance ${index}: Bank ${balance.bankPk.toString()}, Asset shares: ${assetShares.toString()}, Liability shares: ${liabilityShares.toString()}`);
                    }
                }

                // Use the helper function properly - it should handle the bank selection
                await makeKaminoWithdrawThroughMarginfi(userA, usdcBank, collateralAmount, false);

                // Verify withdrawal
                const userBalanceFinal = await getTokenBalance(bankRunProvider, userA.usdcAccount);
                const marginfiAccountFinal = await userA.mrgnBankrunProgram.account.marginfiAccount.fetch(
                    userA.accounts.get(USER_ACCOUNT_K)!
                );
            ...
            it("User B - Deposit and withdraw $500k worth of Token A", async () => {
                ...
                const DEPOSIT_AMOUNT = new BN(tokenAAmount).mul(new BN(10 ** ecosystem.tokenADecimals));
                ...
                // Get initial balances
                const userBalanceBefore = await getTokenBalance(bankRunProvider, userB.tokenAAccount);
                const marginfiAccountBeforeDeposit = await userB.mrgnBankrunProgram.account.marginfiAccount.fetch(
                ...
                await processBankrunTransaction(bankrunContext, depositTx, [userB.wallet]);

                // Get balances after deposit
                const userBalanceAfterDeposit = await getTokenBalance(bankRunProvider, userB.tokenAAccount);
                const marginfiAccountAfterDeposit = await userB.mrgnBankrunProgram.account.marginfiAccount.fetch(
                    userB.accounts.get(USER_ACCOUNT_K)!
                );
                ...
                await makeKaminoWithdrawThroughMarginfi(userB, tokenABank, collateralAmount, false);

                // Get final balances
                const userBalanceFinal = await getTokenBalance(bankRunProvider, userB.tokenAAccount);
    ...
    describe("2. Reserve Refresh Requirement Tests (Expected Failures)", () => {
        it("Verify reserve refresh is required for deposits after time advancement", async () => {
            const DEPOSIT_AMOUNT = new BN(10_000).mul(new BN(10 ** ecosystem.usdcDecimals));
        ...
        it("Verify reserve refresh is required for withdrawals after time advancement", async () => {
            ...
            const marginfiAccountBefore = await userA.mrgnBankrunProgram.account.marginfiAccount.fetch(
                userA.accounts.get(USER_ACCOUNT_K)!
            );
            ...
            const assetSharesBefore = wrappedI80F48toBigNumber(balanceBefore.assetShares);
    ...
    describe("3. Time-Based Interest Accrual Tests", () => {
        it("Deposit USDC, advance time, verify interest accrual", async () => {
            ...
            const userBalanceBeforeDeposit = await getTokenBalance(bankRunProvider, userA.usdcAccount);
            ...
            const collateralTokensBefore = new BN(wrappedI80F48toBigNumber(balanceBefore.assetShares).toString());
            ...
            await makeKaminoWithdrawThroughMarginfi(userA, usdcBank, collateralTokensBefore, false);
    ...
    describe("4. Randomized Deposit/Withdraw Tests", () => {
        it("Random deposits and withdrawals with time advancement", async () => {
            ...
            for (let i = 0; i < NUM_ITERATIONS; i++) {
                ...
                const oracle = isUsdc ? oracles.usdcOracle.publicKey : oracles.tokenAOracle.publicKey;
                const decimals = isUsdc ? ecosystem.usdcDecimals : ecosystem.tokenADecimals;
                const tokenPrice = isUsdc ? 1 : 10;
                const mint = isUsdc ? ecosystem.usdcMint.publicKey : ecosystem.tokenAMint.publicKey;

                // Check current account value (simplified - just checking if position exists)
                const marginfiAccount = await user.mrgnBankrunProgram.account.marginfiAccount.fetch(
                ...
                const hasPosition = existingBalance && wrappedI80F48toBigNumber(existingBalance.assetShares).gt(0);

                // Decide action
                const shouldWithdraw = hasPosition && (Math.random() < 0.3 || i === NUM_ITERATIONS - 1);

                if (shouldWithdraw) {
                    // Withdraw - helper will handle refresh internally
                    const withdrawAll = Math.random() < 0.5;
                    const cTokenAmount = new BN(wrappedI80F48toBigNumber(existingBalance!.assetShares).toString());
                    const withdrawAmount = withdrawAll
                        ? cTokenAmount
                        : cTokenAmount.mul(new BN(40 + Math.floor(Math.random() * 21))).div(new BN(100)); // 40-60%

                    await makeKaminoWithdrawThroughMarginfi(user, bank, withdrawAmount, false);
                } else {
                    ...
                    await processBankrunTransaction(bankrunContext, refreshTx, [globalProgramAdmin.wallet]);
                    // Deposit
                    const depositUsdValue = MIN_DEPOSIT_USD + Math.random() * (MAX_DEPOSIT_USD - MIN_DEPOSIT_USD);
                    const tokenAmount = depositUsdValue / tokenPrice;
                    const depositAmount = new BN(Math.floor(tokenAmount)).mul(new BN(10 ** decimals));
                    ...
                    const depositTx = new Transaction().add(
                        ...
                        await makeKaminoDepositIx(
                            user.mrgnBankrunProgram,
                            {
                                ...
                                obligationFarmUserState: !isUsdc ? farmAccounts.get(A_OBLIGATION_USER_STATE) : null,
                                reserveFarmState: !isUsdc ? farmAccounts.get(A_FARM_STATE) : null,
                            },
                            depositAmount
                        )
                    );
                ...
                const timeAdvance = 3600 + Math.floor(Math.random() * (7 * 24 * 3600 - 3600));
                const currentClock = await banksClient.getClock();
        ...
        describe("5.1 Withdraw All Function", () => {
            it("Deposit various amounts and use isFinalWithdrawal", async () => {
                ...
                let tx = new Transaction()
                    ...
                    .add(await simpleRefreshReserve(
                        klendBankrunProgram,
                        usdcReserve,
                        kaminoAccounts.get(MARKET)!,
                        oracles.usdcOracle.publicKey
                    ))
                    .add(await simpleRefreshObligation(
                        klendBankrunProgram,
                        kaminoAccounts.get(MARKET)!,
                        usdcBankObligation,
                        [usdcReserve]
                    ))
                    .add(await makeKaminoDepositIx(
                        userA.mrgnBankrunProgram,
                        {
                            marginfiAccount: userA.accounts.get(USER_ACCOUNT_K)!,
                            bank: usdcBank,
                ...
                await makeKaminoWithdrawThroughMarginfi(userA, usdcBank, new BN(0), true);

                // Verify position is completely closed
                const finalAccount = await userA.mrgnBankrunProgram.account.marginfiAccount.fetch(
                    userA.accounts.get(USER_ACCOUNT_K)!
                );
                const finalBalance = finalAccount.lendingAccount.balances.find(
                    b => b.bankPk.equals(usdcBank)
                );

                assert.ok(!finalBalance || finalBalance.active === 0, "Position should be completely closed");
            });
        });
    });
    ...
    describe("6. Final Cleanup Verification", () => {
        it("Ensure all positions are fully withdrawn", async () => {
            ...
            while (hasActivePositions) {
                ...
                const currentAccount = await userA.mrgnBankrunProgram.account.marginfiAccount.fetch(
                    userA.accounts.get(USER_ACCOUNT_K)!
                );
                ...
                for (const balance of currentAccount.lendingAccount.balances) {
                    if (balance.active === 1 && wrappedI80F48toBigNumber(balance.assetShares).gt(0)) {
            ...
            while (userBHasActivePositions) {
                ...
                const currentAccountB = await userB.mrgnBankrunProgram.account.marginfiAccount.fetch(
                    userB.accounts.get(USER_ACCOUNT_K)!
                );
            ...
            // Final verification
            const finalAccountA = await userA.mrgnBankrunProgram.account.marginfiAccount.fetch(
                userA.accounts.get(USER_ACCOUNT_K)!
            );
            const finalAccountB = await userB.mrgnBankrunProgram.account.marginfiAccount.fetch(
                userB.accounts.get(USER_ACCOUNT_K)!
            );
    ...
    async function makeKaminoWithdrawThroughMarginfi(
        ...
        const marginfiAccount = await user.mrgnBankrunProgram.account.marginfiAccount.fetch(userAccount);

        // Determine bank-specific details from global account maps
        const isUsdc = bank.equals(kaminoAccounts.get(KAMINO_USDC_BANK)!);
        const reserve = isUsdc ? kaminoAccounts.get(USDC_RESERVE)! : kaminoAccounts.get(TOKEN_A_RESERVE)!;
        const obligation = isUsdc ? usdcBankObligation : tokenABankObligation;
        const oracle = isUsdc ? oracles.usdcOracle.publicKey : oracles.tokenAOracle.publicKey;
        const tokenAccount = isUsdc ? user.usdcAccount : user.tokenAAccount;
        const mint = isUsdc ? ecosystem.usdcMint.publicKey : ecosystem.tokenAMint.publicKey;
        ...
        for (const balance of marginfiAccount.lendingAccount.balances) {
            if (balance.active === 1) {
                ...
                } else if (balance.bankPk.equals(kaminoAccounts.get(KAMINO_TOKENA_BANK)!)) {
                    activePositions.push([
                        balance.bankPk,
                        oracles.tokenAOracle.publicKey,
        ...
        const withdrawTx = new Transaction().add(
            ...
            await makeKaminoWithdrawIx(
                user.mrgnBankrunProgram,
                {
                    ...
                    reserveLiquidityMint: mint,
                    // Add farm accounts for Token A (has farms enabled from k13)
                    obligationFarmUserState: isUsdc ? null : farmAccounts.get(A_OBLIGATION_USER_STATE),

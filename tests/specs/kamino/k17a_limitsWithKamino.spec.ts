...
const ORACLE_MODES = ["pyth", "switchboard"] as const;
...
ORACLE_MODES.forEach((oracleMode, oracleModeIndex) => {
  ...
  // Marginfi-bank oracle for regular and kamino banks. NOTE: the kamino RESERVE refresh
  // (simpleRefreshReserve) keeps the reserve's own pyth oracle regardless of mode.
  const getRegularOraclePk = () =>
    oracleMode === "switchboard"
      ? oracles.lstAlphaOracleSwb.publicKey
      : oracles.pythPullLst.publicKey;
  const getKaminoOraclePk = () =>
    oracleMode === "switchboard"
      ? oracles.tokenAOracleSwb.publicKey
      : oracles.tokenAOracle.publicKey;

  describe(`k14: Limits on number of accounts with Kamino and emode [${oracleMode}]`, () => {
    ...
    it("init group, init banks, and fund banks", async () => {
      const result = await genericMultiBankTestSetup(
        ...
        groupBuff,
        startingSeed + oracleModeIndex * 1000,
        KAMINO_POSITIONS + 1,
        0,
        oracleMode
      );
    ...
    it("(user 0) Borrows 15 positions against 1 - validates max borrows possible", async () => {
      ...
      for (let i = 1; i < regularBanks.length; i += 1) {
        ...
        for (let k = 1; k <= i; k++) {
          remainingAccounts.push([regularBanks[k], getRegularOraclePk()]);
        }
    ...
    it("(user 2) Liquidates user 0", async () => {
      ...
      for (let i = 1; i < MAX_BALANCES; i++) {
        remainingAccounts.push([regularBanks[i], getRegularOraclePk()]);
      ...
      tx = new Transaction().add(
        ...
        await liquidateIx(liquidator.mrgnBankrunProgram, {
          ...
          remaining: [
            getKaminoOraclePk(), // asset oracle
            tokenAReserve,
            getRegularOraclePk(), // liab oracle

            ...composeRemainingAccounts([
              // liquidator accounts
              [regularBanks[0], getRegularOraclePk()],
              // We pick this up as an asset
              [kaminoBanks[0], getKaminoOraclePk(), tokenAReserve],
              // We pick this up as a liability
              [regularBanks[MAX_BALANCES - 1], getRegularOraclePk()],
            ]),

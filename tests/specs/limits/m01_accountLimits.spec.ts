...
ORACLE_MODES.forEach((oracleMode, oracleModeIndex) => {
  // Distinct 32-byte group seed + account name per mode so the two runs don't collide.
  const groupBuff = Buffer.from(
    oracleMode === "switchboard"
      ? "MARGINFI_GROUP_SEED_1234000000MS"
      : "MARGINFI_GROUP_SEED_1234000000MP"
  );
  const USER_ACCOUNT_THROWAWAY = `throwaway_account1_${oracleMode}`;
  const getLstOraclePk = () =>
    oracleMode === "switchboard"
      ? oracles.lstAlphaOracleSwb.publicKey
      : oracles.pythPullLst.publicKey;

  describe(`m01: Limits on number of accounts [${oracleMode}]`, () => {
    it("init group, init banks, and fund banks", async () => {
      const result = await genericMultiBankTestSetup(
        MAX_BALANCES,
        USER_ACCOUNT_THROWAWAY,
        groupBuff,
        startingSeed + oracleModeIndex * 100,
        0,
        0,
        oracleMode
      );

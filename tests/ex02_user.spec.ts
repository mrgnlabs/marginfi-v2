...
describe("exponent::user", () => {
  it("creates a user yield position instruction", async () => {
    ...
    const ownerMeta = ix.keys.find((k) => k.pubkey.equals(user.wallet.publicKey));
    assert.isTrue(ownerMeta?.isSigner ?? false, "owner should sign yield init instruction");
  });
  ...
  it("allows a second user to buy PT for fixed yield", async () => {
    ...
    assert.equal(ix.programId.toBase58(), program.programId.toBase58());
    const traderMeta = ix.keys.find((k) => k.pubkey.equals(fixedBuyer.wallet.publicKey));
    assert.isTrue(traderMeta?.isSigner ?? false, "fixed buyer should sign PT trade instruction");
  });
});

export const exponentUserState = {

...
async function createReserve(
  ...
  const [collateralSupply] = reserveCollateralSupplyPda(
    market,
    mint,
    programId
  );
...
/**
 * If a user doesn't have a Kamino-native metadata and obligation, creates one.
 * @param owner
 * @param obligationId
 * @returns
 */
async function ensureKaminoUser(
  ...
  const [metadata] = deriveUserMetadata(
    klendBankrunProgram.programId,
    owner.publicKey
  );
  try {
    await bankRunProvider.connection.getAccountInfo(metadata);
  } catch (e) {
...
async function mintToAccount(
  ...
  await processBankrunTransaction(bankrunContext, new Transaction().add(ix), [
...
describe("k18: Kamino interest after borrow", () => {
  ...
  it("deposits, borrows, and accrues interest correctly", async () => {
    ...
    const market = kaminoAccounts.get(MARKET)!;
    const tokenAMint = ecosystem.tokenAMint.publicKey;
    const tokenADecimals = ecosystem.tokenADecimals;
    const tokenAOracle = oracles.tokenAOracle.publicKey;

    const tokenBBaseUnit = bn10.pow(new BN(tokenBDecimals));
    const reserveDepositAmount = new BN(1_000).mul(tokenBBaseUnit);
    const marginfiDepositAmount = new BN(150).mul(tokenBBaseUnit);
    const tokenABaseUnit = bn10.pow(new BN(tokenADecimals));
    const collateralDepositAmount = new BN(1_000).mul(tokenABaseUnit);
    ...
    // TODO create a new market and reserve A instead of recycling this one...
    const collateralReservePubkey = kaminoAccounts.get(TOKEN_A_RESERVE);
    ...
    const borrowUser = users[3];
    const borrowObligation = await ensureKaminoUser(borrowUser.wallet, 92);
    await mintToAccount(
      collateralDepositAmount,
      tokenAMint,
      borrowUser.tokenAAccount
    );

    const collateralReserveBeforeDeposit =
      await klendBankrunProgram.account.reserve.fetch(collateralReservePubkey);
    const collateralLiquidityMint =
      collateralReserveBeforeDeposit.liquidity.mintPubkey;
    const collateralLiquiditySupply =
      collateralReserveBeforeDeposit.liquidity.supplyVault;
    const collateralMintA =
      collateralReserveBeforeDeposit.collateral.mintPubkey;
    const collateralSupplyA =
      collateralReserveBeforeDeposit.collateral.supplyVault;

    console.log("Before second deposit");
    const collateralDepositTx = new Transaction().add(
      await simpleRefreshReserve(
        klendBankrunProgram,
        collateralReservePubkey,
        market,
      ...
      await klendBankrunProgram.methods
        .depositReserveLiquidityAndObligationCollateral(collateralDepositAmount)
        .accounts({
          owner: borrowUser.wallet.publicKey,
          obligation: borrowObligation,
          lendingMarket: market,
          lendingMarketAuthority,
          reserve: collateralReservePubkey,
          reserveLiquidityMint: collateralLiquidityMint,
          reserveLiquiditySupply: collateralLiquiditySupply,
          reserveCollateralMint: collateralMintA,
          reserveDestinationDepositCollateral: collateralSupplyA,
          userSourceLiquidity: borrowUser.tokenAAccount,
          placeholderUserDestinationCollateral: null,
          collateralTokenProgram: TOKEN_PROGRAM_ID,
          liquidityTokenProgram: TOKEN_PROGRAM_ID,
          instructionSysvarAccount: SYSVAR_INSTRUCTIONS_PUBKEY,
    ...
    const borrowAmount = collateralDepositAmount.divn(100);
    ...
    const borrowTx = new Transaction().add(
      ...
      await klendBankrunProgram.methods
        ...
        .accounts({
          ...
          userDestinationLiquidity: borrowUser.tokenBAccount,
          borrowReserveLiquidityFeeReceiver:
            reserveAfterDeposit.liquidity.feeVault,
    ...
    const reserveAfterBorrow = await klendBankrunProgram.account.reserve.fetch(
      reservePubkey
    );
    ...
    await mintToAccount(
      marginfiDepositAmount.mul(new BN(2)),
      tokenBMint,
      marginfiUser.tokenBAccount
    );
    ...
    await processBankrunTransaction(bankrunContext, kaminoDepositTx, [
      marginfiUser.wallet,
    ]);
    ...
    const marginfiAccountAfterPulse =
      await bankrunProgram.account.marginfiAccount.fetch(
        marginfiAccountKeypair.publicKey
      );
    const bankAfterPulse = await bankrunProgram.account.bank.fetch(bankPk);
    const reserveForInitialValuation =
      await klendBankrunProgram.account.reserve.fetch(reservePubkey);

    const balanceRecord =
      marginfiAccountAfterPulse.lendingAccount.balances.find(
        (balance) => balance.bankPk.equals(bankPk) && balance.active === 1
      );
    assert.isDefined(
      balanceRecord,
      "marginfi position should exist after deposit"
    );

    const assetShares = wrappedI80F48toBigNumber(balanceRecord!.assetShares);
    const bankShareValueInitial = wrappedI80F48toBigNumber(
      bankAfterPulse.assetShareValue
    );
    ...
    await processBankrunTransaction(bankrunContext, refreshAfterWarp, [
      groupAdmin.wallet,
    ]);
    ...
    const reserveAfterInterest =
      await klendBankrunProgram.account.reserve.fetch(reservePubkey);
    const bankAfterInterest = await bankrunProgram.account.bank.fetch(bankPk);
    const bankShareValueAfter = wrappedI80F48toBigNumber(
      bankAfterInterest.assetShareValue
    );

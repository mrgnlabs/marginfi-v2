...
describe("k14: Limits on number of accounts, with Kamino and emode", () => {
  ...
  it("(admin) Tries to deposit into 9th Kamino bank - should fail with KaminoPositionLimitExceeded", async () => {
    ...
    const hasKaminoLimitError = logs.some(
      (msg) => msg.includes("0x1844") // 6212 in hex
    );

    if (!hasKaminoLimitError) {
      throw new Error(
        "Expected KaminoPositionLimitExceeded error (0x1844 / 6212), got different error:\n" +
          logs.join("\n")
      );
    }
  });

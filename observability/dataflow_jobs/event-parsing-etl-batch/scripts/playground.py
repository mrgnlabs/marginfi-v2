...
from event_parsing_etl_batch.transaction_log_parser import reconcile_instruction_logs, Instruction
...
sample_inner_ixs = [
    {"index": 1, "instructions": [{"programIdIndex": 9, "accounts": [5, 1, 4], "data": "3QCwqmHZ4mdq"}]}]
            ...
            for ix_raw in inner_instructions[inner_ixs_index]["instructions"]:
                compiled_instructions.append(
                    CompiledInstruction(program_id_index=ix_raw["programIdIndex"], accounts=bytes(ix_raw["accounts"]),
                                        data=base64.b64decode(ix_raw["data"])))
...
for ix in ix_with_logs:
    print("\n\n=============================================")
    print("truncated?", ix.logs_truncated)
    for log in ix.logs:
        print(log)

...
/// Optional carry-exit policy on an `Order`, orthogonal to its price trigger. Each field sets the
/// `Order` field of the same name; `None` takes the matching `INTEREST_DEFAULT_*` constant.
#[repr(C)]
#[cfg_attr(feature = "anchor", derive(AnchorSerialize, AnchorDeserialize))]
#[derive(Debug, PartialEq, Eq, Copy, Clone)]
pub struct InterestTriggerConfig {
    pub window_seconds: Option<u32>,
    pub exit_budget_seconds: Option<u32>,
